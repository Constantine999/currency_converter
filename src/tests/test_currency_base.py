...
class TestCurrentBaseExchangeRates:
    """
    Проверка объекта current_base_exchange_rates из модуля exchange_rates
    """
    ...
    async def test_current_base_exchange_rates_on_equal_names_with_tickers(self, ac: AsyncClient):
        ...
        """
        assert sorted(TickersEnum.__dict__["_member_names_"]) == sorted(current_base_exchange_rates.keys())

    async def test_current_base_exchange_rates_on_on_different_names_with_tickers(self, ac: AsyncClient):
        """
        Тест на проверку имен ключей словаря current_base_exchange_rates с значениями объекта TickersEnum
        """
        assert sorted(TickersEnum.__dict__["_member_names_"] + ["SOS"]) != sorted(current_base_exchange_rates.keys())
    ...
    async def test_current_base_exchange_rates_on_type_value(self, ac: AsyncClient):
        ...
        """
        assert all(isinstance(value, Decimal) for value in current_base_exchange_rates.values()) is True
    ...
    async def test_current_base_exchange_rates_on_three_symbols_in_key(self, ac: AsyncClient):
        ...
        """
        assert all(len(key) == 3 for key in current_base_exchange_rates) is True
